        ...
        function Pin-App ([string]$appname, [switch]$unpin, [switch]$start, [switch]$taskbar, [string]$path) {
            ...
            if ($taskbar.IsPresent) {
                try {
                    $exec = $false
                    if ($action -eq "Unpin") {
                        ((New-Object -Com Shell.Application).NameSpace('shell:::{4234d49b-0245-4df3-b780-3893943456e1}').Items() | ?{$_.Name -ne $appname}).Verbs() | ?{$_.Name.replace('&','') -match 'Unpin from taskbar'} | %{$_.DoIt(); $exec = $true}
                        if ($exec) {
            ...
            if ($start.IsPresent) {
                try {
                    $exec = $false
                    if ($action -eq "Unpin") {
                        ((New-Object -Com Shell.Application).NameSpace('shell:::{4234d49b-0245-4df3-b780-3893943456e1}').Items() | ?{$_.Name -ne $appname}).Verbs() | ?{$_.Name.replace('&','') -match 'Unpin from Start'} | %{$_.DoIt(); $exec = $true}
                        ...
                        if ($exec) {
                            ...
                            if (-not $path -eq "") {
                                ...
                                #needs improvement - sometimes start menu items don't get unpinened. culprit is based on looking for $appname just like up above ^^^
...
#########Start the downloads themselves:#################################################################################
        ...
        # needs improvement - will launch multiple installation files if multiple are downloaded | change to only 1 instance
        # Start the setup.exes for installation
...
#>
<#
# needs improvement - do this automatically
...
# Disable allow remote assistance connections to this computer automatically - needs improvement - not working automatically
