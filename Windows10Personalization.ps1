...
########################################
# Source(s): 
# https://github.com/Disassembler0/Win10-Initial-Setup-Script | Disassembler <disassembler@dasm.cz>
# Google
# Others linked as needed
# Author: Austin Esser
########################################
#
...
#    $manualStepsMain
    ...
    "configureSystemProperties", # first time setup only
#    "privacyManualSteps", # always run 
#    "configureUserFolders", # always run
...
#   Start menu pinning ------- (explorer/win 81 UI)
    ...
    Function unpinTaskbarAndStartMenuTiles {
        # Start Menu Reg
        Write-Host "Unpinning all Start Menu tiles via Registry..." -ForegroundColor Green -BackgroundColor Black
        ...
        # This might cause errors with newer versions of windows 10
        ...
        #TaskBar Reg
        Write-Host "Unpinning all Taskbar icons via Registry..." -ForegroundColor Green -BackgroundColor Black
...
#   Rename computer to something a little more personal
...
#   Advanced system settings - (explorer)
    ...
    Function pinStartMenuItemsAndInstallSoftware {
        ...
        do {
            $doneUser = $false
            $SoftwareType = Read-Host -Prompt "Select a web browser: [0]IE/Edge | [1]Chrome | [2]FireFox"
            ...
            elseif ($SoftwareType -eq "2"){
                ...
                $doneUser = $false
                Write-Host "Select a web browser to install: [0]IE/Edge | [1]Chrome | [2]FireFox" -ForegroundColor Red -BackgroundColor Black
            }
        }
        ...
        # left off here, tackle these first
        # needs improvement, skip browser download if the installer is already downloaded (note: the install is already skipped but not the download)
        # needs improvement, if internet exploerer isn't installed (on home editions of windows, then this won't launch the chrome download! going to have to Invoke-Item M$Edge as an alternative)
        if ($chrome -eq $true){
        ...
        elseif ($firefox -eq $true){
            ...
            Write-Host "Setting internet exploder as the default downloader for software" -ForegroundColor Yellow -BackgroundColor Black
            $browserPath = "C:\Program Files (x86)\Internet Explorer\iexplore.exe"
            $browserSelection = "Internet Exploder"
        }
        ...
        elseif ($ie -eq $true) {
            Write-Host "Using internet exploder" -ForegroundColor Green -BackgroundColor Black
        }
        else {
...
#########Start the downloads themselves:#################################################################################
        ...
        if ($spotify -and !(Test-Path "C:\Users\$env:username\AppData\Roaming\Spotify\Spotify.exe")) {
            #needs improvement - need to install spotify using a normal account instead of an Admin account (script is running as admin)
            Invoke-Item "C:\Users\$env:username\Downloads\*spotify*"            
        }
        ...
        # needs improvement, exit out of ONLY the downloads folder when the last piece of software is installed, try not to kill explorer itself
        #                    | Where Sub-Process -Name -like "*Downloads*" | Stop-Process
        ...
        #Set default apps - needs improvement, could possibly set these automatically if ($appOfChoice -eq $true)? 
...
# Disable teamviewer service if you're not listening for connections on this computer - only if the user is not going to be listening for remote connections
# Needs improvement - possible to determine enabled/disabled automatically?
# Needs improvement - some condition isn't set correctly and this isn't getting triggered. I'm guessing $teamviewer is the culprit?
if ($firstTimeManualMainCall -and $teamviewer) {
...
# Improvements needed:
# Configurations:
# Skip over launching browser installer if browser is already installed
...
#
# Errors:
# Spotify software install requires the .exe to be invoked as the current user context
# Apps (recently uninstalled) show up on start menu as 'new' - remove them
#
# Script: 
# Check if execution-policy has been permenantly changed after script is run
...
#>
<#
# Configure notification center focus assist - gaming/duplicating - uncheck "show a notification in action center when FA is on automatically"
...
# Disable allow remote assistance connections to this computer automatically - needs improvement - not working
Function disableRemoteAssistance {
    # check this HKLM\SYSTEM\CurrentControlSet\Control\Remote Assistance
    #if (!(Test-Path "HKLM:\SYSTEM\CurrentControlSet\ControlTerminal Server\")){
    #    New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\ControlTerminal Server\" -Force
    #}
    #Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\ControlTerminal Server\" -Name "fAllowToGetHelp" -Type DWord -Value 0
}
# needs improvement - get/list drives automatically? or open user folder properties,location if more than one drive is detected?
Function configureUserFolders { 
    ...
    if ($fixedDriveCount -gt 1 -and #non-C-drive)
...
# Remove Recycle Bin from the desktop - only after it has been pinned to the start menu
...
# Call removeRecycleBin function
removeRecycleBin
#>
######################################################################################################################################################
<#
...
#Extra stuff I'm not sure where to put | Needs improvement - finish building out and put functions in the correct place
#Start menu categories
Pause
Write-Host "Start Menu App Categories:" -ForegroundColor Yellow -BackgroundColor Black
Write-Host "Applications" -ForegroundColor Green -BackgroundColor Black
Write-Host "System Tools" -ForegroundColor Green -BackgroundColor Black
Write-Host "Navigation" -ForegroundColor Green -BackgroundColor Black
Write-Host "Pin 'Recycle Bin' and 'Downloads' manually" -ForegroundColor Red -BackgroundColor Black
Write-Host " "

#A reminder for myself
Write-Host "Also map network drives:" -ForegroundColor Yellow -BackgroundColor Black
Write-Host "V: Store 192.168.1.106 | aesser FreeNAS user" -ForegroundColor Green -BackgroundColor Black
Write-Host "W: Media 192.168.1.106 | aesser FreeNAS user" -ForegroundColor Green -BackgroundColor Black
Write-Host "X: NVR   192.168.1.106 | aesser FreeNAS user" -ForegroundColor Green -BackgroundColor Black
Write-Host "Y: Share 192.168.1.106 | aesser FreeNAS user" -ForegroundColor Green -BackgroundColor Black
Write-Host "Z: Watch 192.168.20.20 | mediasvc01 MediaSrv user" -ForegroundColor Green -BackgroundColor Black
Write-Host " "
Write-Host "Pin FreeFileSyncBackupLogs and Torrents watch directory to quick access" -ForegroundColor Yellow -BackgroundColor Black
Pause
#>